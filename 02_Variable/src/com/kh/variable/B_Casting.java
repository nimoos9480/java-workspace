...
//		b.autoCasting();
	 ...
	 * - 데이터 범위가 작은 자료형을 큰 자료형으로 (알아서) 변환한다.
	 * */
	...
	public void casting() {
		double d = 4.12345678904567890; // 소수점 15자리수까지만 표현 가능
		float f = (float) d; // 소수점 6자리수까지 표현 가능
		int i = (int) f; // i : 4 출력 = 데이터 손실 발생!
		...
		int number = 290;
		byte bNumber = (byte)number; // -128 ~ 127까지만 출력
		
		System.out.println(bNumber);
		
		int number2 = 128;
		byte bNumber2 = (byte)number2; // 34 출력 
		System.out.println(bNumber2); // -128 출력됨(범위 초과되면 128 ~ 127까지 돌아가면서 출력되는 것)
		
		// char <-> int : 각 문자들마다 고유한 숫자가 지정되어 있기 때문에 쌍방향으로 형변환 가능(유니코드)
		// char는 음수값 저장 불가 -> 값의 범위가 0~65535까지
		int num = 'A'; // 형 다르지만 입력됨
