...
//		a.method1();
	...
	public void method1() {
		...
		result = Arrays.stream(values).allMatch(value -> value % 2 == 0);   // 2의 배수니?
		System.out.println(result);   // true 출력
		
		result = Arrays.stream(values).anyMatch(value -> value % 3 == 0); // 3의 배수니?
		System.out.println(result); // true 출력
		
		result = Arrays.stream(values).noneMatch(value -> value % 5 == 0); // 5의 배수니?
		System.out.println(result); // true 출력
	...
	public void method2() {
		...
		System.out.println("남학생들 중에 평균이 70점 이상인 학생이 한 명이라도 존재하는가? : " + result);
	}
