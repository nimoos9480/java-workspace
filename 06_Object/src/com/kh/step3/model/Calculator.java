...
public class Calculator {
	...
	public static int multiply(int a, int b) { // 스태틱은 인스턴스변수, 인스턴스메서드 사용 못함 => 매개변수 사용
		return a*b;
		}
	
	// 나누기 -> 형태 : "몫은 1, 나머지는 3이다."
	public static String divide(int a, int b){
		return "몫은 " + quotient(a,b) +", 나머지는 " + remainder(a,b);
		
		}
	
	public static int quotient(int a, int b) { // 나머지
		return a/b;
	}
	
	public static int remainder(int a, int b) { // 몫
		return a%b;
	}
	...
	public int factorial(int n) {
		...
		int fac = 1;
		if(n!=1) fac = n * factorial(n-1);  // 재귀법 : 본인의 메소드를 본인이 부르는 것(for문 지옥에 빠지는 것 방지)
